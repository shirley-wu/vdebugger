...
class GQADataset(Dataset):
    ...
    def __init__(self, split, balanced=True, data_path="", image_transforms=None, question_transforms=None,
                 tokenize=None, verbose=False, testing=False, max_samples=None, first_n=None, return_pil=True,
                 extra_context=None, sample_n=None, start_sample=None, load_image=False, orig_query=False, **kwargs):
        """
        Args:
        ...
        # assert split in ["challenge", "submission", "test", "testdev", "testdev-1000subset", "train", "val"]
        ...
        if max_samples is not None:
            if sample_n is not None:
                print("Warning: sample first, then select max")
            self.df = self.df[:max_samples]
        if start_sample is not None:
            self.df = self.df[start_sample:]
        print("Data frame shape:", self.df.shape)

        if extra_context is not None:
            if 'code_for_correction' in extra_context:
                df_for_correction = pd.read_csv(extra_context['code_for_correction'])
                if len(df_for_correction) > len(self.df):
                    print("Emmmmm, your provided code df has %d rows. It's larger than %d. Will truncate" % (
                        len(df_for_correction), len(self.df)))
                    df_for_correction = df_for_correction[:len(self.df)]

                extra_context_list = ["def execute_command(image) -> str:" + x for x in
                                      df_for_correction['code'].tolist()]

                if extra_context.get("trace", False):
                    for i in range(len(extra_context_list)):
                        extra_context[i] = '{}\n\n--- Trace\n\n{}'.format(
                            extra_context_list[i], df_for_correction['traced'].iloc[i]
                        )

                self.df.insert(len(self.df.columns), 'extra_context', extra_context_list)
            else:
                raise KeyError
    ...
    def __getitem__(self, index):
        ...
        extra_context = self.df['extra_context'].iloc[index] if 'extra_context' in self.df else None
        ...
        if extra_context is not None:
            out_dict['extra_context'] = extra_context
        return out_dict
